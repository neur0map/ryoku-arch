...
assert_local_source_uses_checkout_even_when_ryoku_path_is_set() {
  local temp_dir project_dir bin_dir home_dir shell_dir log_file expected_mount expected_shell_mount status

  temp_dir=$(mktemp -d)
  project_dir="$temp_dir/project"
  bin_dir="$temp_dir/bin"
  home_dir="$temp_dir/home"
  shell_dir="$temp_dir/ryoku-shell"
  log_file="$temp_dir/docker-args.log"
  status=0

  mkdir -p "$project_dir/iso/bin" "$project_dir/iso/builder" "$project_dir/iso/configs" "$bin_dir" "$home_dir" "$temp_dir/stale" "$shell_dir/.git"
...
EOF
  ...
  expected_shell_mount="$shell_dir:/ryoku-shell:ro"

  PATH="$bin_dir:$PATH" \
    HOME="$home_dir" \
    RYOKU_PATH="$temp_dir/stale" \
    RYOKU_SHELL_SOURCE="$shell_dir" \
  ...
  if ! grep -F -- "$expected_shell_mount" "$log_file" >/dev/null; then
    cat "$log_file" >&2
    rm -rf "$temp_dir"
    fail "--local-source should mount a local Ryoku shell checkout when available"
  fi
