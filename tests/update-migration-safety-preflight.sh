...
assert_contains "$PERFORM" 'canonical_shell_runtime_dir\(\)' \
  "update performer should define the canonical shell runtime instead of inheriting stale host-shell paths"
assert_contains "$PERFORM" 'RYOKU_SHELL_RUNTIME_DIR="\$\(canonical_shell_runtime_dir\)"' \
  "update performer should force shell deployment into ~/.config/quickshell/ryoku-shell"
assert_contains "$PERFORM" '-u QS_CONFIG_NAME' \
  "update performer should clear stale Quickshell environment before shell deployment"
...
assert_order "$PERFORM" 'ryoku-migrate --repair-baseline' 'ryoku_update_run_stage 8 .*"Migrations"' \
