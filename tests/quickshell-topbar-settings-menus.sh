...
for path in "$shell" "$popups" "$topbar" "$control_panel" "$popup_dismiss" "$layer" "$system_popup" "$settings_popup" "$ipc" "$bindings"; do
...
grep -q 'Popups.wallpaperMode = "wallpaper"' "$settings_popup" \
  || fail "SettingsMenuPopup should open the wallpaper selector"
grep -q 'Popups.wallpaperMode = "theme"' "$settings_popup" \
  || fail "SettingsMenuPopup should open the theme selector"
