...
grep -q "nothing was changed" "$out" || fail "dry-run should announce it changed nothing"
grep -q "will NOT touch" "$out" || fail "dry-run plan should list what is never touched"
grep -q "deploy complete" "$out" || fail "dry-run should preview the deploy stage end to end"
