...
| `preflight.sh` | `preflight` | Gates before any disk write: root, UEFI, Secure Boot off, `RYOKU_DISK` a whole disk >= 32 GiB, the repo payload present; for `alongside` also GPT, exactly one EF00 holding `/EFI/Microsoft` with >= 8 MiB free, a free region >= `2 + 20 + swap` GiB, and a BitLocker-neighbor warning. | Nothing. | Fatal `die` with firmware/guidance text; dry-run narrates and returns. |
| `disk.sh` | `partition` | Partitions the target: `whole` (wipe, fresh GPT ESP + root) or `alongside` (a 2 GiB XBOOTLDR boot + Btrfs root at explicit sectors in the chosen free region; Windows' ESP is shared, never wiped); also serves the read-only `probe alongside` verb (the sfdisk free-region + Windows-ESP report the TUI renders), frees a busy disk, wipes stale signatures. | GPT table on `RYOKU_DISK`; sets `ESP_DEV` / `ROOT_PART`. | Fatal. Hard safety proves each new partition before any `wipefs`/`mkfs` and aborts otherwise. |
...
| `bootloader.sh` | `bootloader` (+ post-AUR finalize) | Installs and brands Limine and builds the kernel cmdline. Whole disk: writes the ESP-root `limine.conf`, drops the EFI binary + registers NVRAM, and after the AUR step repoints the menu at the tool-managed UKI tree. Alongside: kernels on the XBOOTLDR `/boot`, then shares Windows' ESP -- tars the ESP to a backup, drops `/EFI/ryoku/BOOTX64.EFI` + `limine.conf` (kernels by FAT label `fslabel(RYOKUBOOT)`, `boot()` Windows chainload), never touching `/EFI/Microsoft`; `finalize` is a no-op there. | `limine.conf` (wipe: `/mnt/boot`; alongside: the Windows ESP), `/etc/default/limine`, `mkinitcpio.conf.d/ryoku-vmd.conf`, the EFI binaries, the NVRAM entry, the alongside ESP backup tar. | Fatal for the core install; `efibootmgr` is best-effort; wipe `finalize` is atomic file surgery. |
