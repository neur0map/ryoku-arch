...
Singleton {
  ...
  /*
  Ryoku runtime directories.
  - Default config directory: $XDG_CONFIG_HOME/ryoku/noctalia-settings
  - Default cache directory: $XDG_CACHE_HOME/ryoku/noctalia-settings
  */
  readonly property alias data: adapter  // Used to access via Settings.data.xxx.yyy
  readonly property int settingsVersion: 59
  property bool isDebug: Quickshell.env("RYOKU_NOCTALIA_DEBUG") === "1"
  readonly property string settingsRelativePath: "ryoku/noctalia-settings/settings.json"
  readonly property string configBaseDir: Quickshell.env("XDG_CONFIG_HOME") || Quickshell.env("HOME") + "/.config"
  readonly property string cacheBaseDir: Quickshell.env("XDG_CACHE_HOME") || Quickshell.env("HOME") + "/.cache"
  readonly property string configDir: ensureTrailingSlash(Quickshell.env("RYOKU_NOCTALIA_CONFIG_DIR") || (configBaseDir + "/ryoku/noctalia-settings"))
  readonly property string cacheDir: ensureTrailingSlash(Quickshell.env("RYOKU_NOCTALIA_CACHE_DIR") || (cacheBaseDir + "/ryoku/noctalia-settings"))

  readonly property string settingsFile: Quickshell.env("RYOKU_NOCTALIA_SETTINGS_FILE") || (configBaseDir + "/" + settingsRelativePath)
  ...
  JsonAdapter {
    ...
    property JsonObject osd: JsonObject {
      ...
      property bool overlayLayer: true
      property real backgroundOpacity: 1.0
      property list<var> enabledTypes: [0, 1, 2]
  ...
  function generateDefaultSettings() {
    Logger.i("Settings", "Default settings generation is disabled in the Ryoku runtime");
    return;
  ...
  function generateWidgetDefaultSettings() {
    Logger.i("Settings", "Default widget settings generation is disabled in the Ryoku runtime");
    return;
  ...
  function runVersionedMigrations(rawJson) {
    Logger.i("Settings", "Versioned migrations are disabled for Ryoku-managed settings");
    return;
  ...
  function upgradeSettings() {
    Logger.i("Settings", "Settings upgrade checks are disabled in the Ryoku runtime");
    return;
