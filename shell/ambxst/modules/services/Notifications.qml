...
Singleton {
    ...
    // Map a ryoku NotifData urgency int → the Ambxst string form.
    ...
    // Project one ryoku NotifData into the Ambxst Notif shape. Carries `_src` so
    ...
    // dnd bridge: Ambxst's `silent` ⇆ ryoku's Notifs.dnd.
    ...
    function notifyInternal(options) {
        ...
        const notificationId = "ambxst-internal-" + (root.internalIdCounter++);
        const obj = {
            "id": notificationId,
            "_src": null,
            "notification": null,
