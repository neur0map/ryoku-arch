...
Singleton {
  ...
  function resolvePath(soundPath) {
    ...
    if (!soundPath.includes("/") && !soundPath.startsWith("file://")) {
      resolvedPath = Quickshell.shellDir + "/noctalia" + "/Assets/Sounds/" + soundPath;
    } else if (!soundPath.startsWith("/") && !soundPath.startsWith("file://")) {
      // Relative path - assume it's relative to shellDir
      resolvedPath = Quickshell.shellDir + "/noctalia" + "/" + soundPath;
    ...
    const playerQml = `
      ...
      MediaPlayer {
        ...
        onErrorOccurred: {
          ...
          if (shouldFallback) {
            const fallbackPath = Quickshell.shellDir + "/noctalia" + "/Assets/Sounds/notification.mp3";
