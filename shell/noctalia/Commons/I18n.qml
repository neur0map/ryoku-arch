...
Singleton {
  ...
  property FileView translationFile: FileView {
    ...
    onLoaded: {
      try {
        ...
        if (root.langCode !== "en") {
          fallbackFileView.path = `file://${Quickshell.shellDir + "/noctalia"}/Assets/Translations/en.json`;
        }
      } catch (e) {
  ...
  function loadTranslations() {
    if (langCode === "")
      return;
    const filePath = `file://${Quickshell.shellDir + "/noctalia"}/Assets/Translations/${langCode}.json`;
