...
ColumnLayout {
  ...
  Component.onCompleted: {
    ...
    if (root.isGitVersion) {
      ...
      if (HostService.isNixOS) {
        var shellDir = Quickshell.shellDir + "/noctalia" || "";
  ...
  Process {
    id: fastfetchProcess
    command: ["fastfetch", "--format", "json", "--config", Quickshell.shellDir + "/noctalia" + "/Assets/Services/fastfetch/system-info.jsonc"]
    running: false
  ...
  // RYOKU: removed Noctalia changelog-on-startup toggle (no ryoku backend).
  ...
  // RYOKU: removed Noctalia telemetry + privacy-policy section (ryoku has no telemetry).
}
