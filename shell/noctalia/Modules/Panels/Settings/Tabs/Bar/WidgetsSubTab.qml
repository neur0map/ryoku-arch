...
ColumnLayout {
  ...
  // RYOKU WIRED: GlobalConfig.bar.entries (the ordered bar layout) + per-widget-type
  // config in GlobalConfig.bar.<widget>.* (barconfig.hpp). BAR widgets only — desktop
  // widgets are configured under Settings → Desktop Widgets.
