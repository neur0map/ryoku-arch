...
migration_check() {
  ...
  if command -v systemctl >/dev/null 2>&1; then
    if ! systemctl --user is-enabled --quiet ryoku-shell.service >/dev/null 2>&1; then
      return 0
    fi
  fi
...
PY
  ...
  if [[ ! -f "$service_file" ]]; then
    service_asset="${REPO_ROOT}/assets/systemd/ryoku-shell.service"
    if [[ -f "$service_asset" ]]; then
      mkdir -p "${xdg_config_home}/systemd/user"
      mkdir -p "${XDG_CACHE_HOME:-$HOME/.cache}"
      tmp_file="${XDG_CACHE_HOME:-$HOME/.cache}/ryoku-shell.service.migration.$$"
  ...
  if command -v systemctl >/dev/null 2>&1 && [[ -f "$service_file" ]]; then
    ...
    systemctl --user enable ryoku-shell.service >/dev/null 2>&1 || true
