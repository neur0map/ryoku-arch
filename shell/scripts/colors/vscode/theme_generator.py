...
THEME_NAME = "Ryoku Material"
THEME_EXTENSION_ID = "ryoku-material-theme"
THEME_FILE_NAME = "ryoku-material-color-theme.json"
PREV_THEME_KEY = "ryoku.previousColorTheme"
STALE_EXTENSION_DIRS = ["i" + "nir.i" + "nir-theme-1.0.0"]
...
def _is_dark_theme(colors: dict) -> bool:
    bg = colors.get("background", colors.get("surface", "#080809"))
    _, _, lightness = _hex_to_hsl(bg)
    return lightness < 0.5


def _write_extension_manifest(ext_dir: Path) -> None:
    manifest = {
        "name": THEME_EXTENSION_ID,
        "displayName": THEME_NAME,
        "description": "Dynamic Material You theme generated by Ryoku from wallpaper colors",
        "version": "1.0.0",
        "publisher": "ryoku",
...
def _write_theme_file(
    theme_path: Path, workbench_colors, token_rules, semantic_rules, colors: dict
) -> None:
    theme_path.parent.mkdir(parents=True, exist_ok=True)
    theme = {
        "$schema": "vscode://schemas/color-theme",
        "name": THEME_NAME,
        "type": "dark" if _is_dark_theme(colors) else "light",
        "colors": workbench_colors,
        "tokenColors": token_rules,
        "semanticHighlighting": True,
        "semanticTokenColors": semantic_rules,
    }
    data = json.dumps(theme, indent=2, ensure_ascii=False) + "\n"
    tmp = theme_path.with_suffix(".tmp")
    with open(tmp, "w") as f:
        f.write(data)
    tmp.rename(theme_path)


def _clean_stale_extensions(fork_key: str) -> None:
    import shutil

    ext_base = _get_ext_dir(fork_key)
    for stale in STALE_EXTENSION_DIRS:
        stale_dir = ext_base / stale
        if stale_dir.is_dir():
...
def _ensure_extension_registered(fork_key: str, ext_dir: Path) -> None:
    ...
    ext_id = f"ryoku.{THEME_EXTENSION_ID}"
    for entry in entries:
        if entry.get("identifier", {}).get("id") == ext_id:
            return
...
def generate_vscode_theme(colors_json_path, scss_path, settings_path, fork_key=""):
    """Install a local extension and write the watched theme file."""
    try:
    ...
    workbench_colors = generate_vscode_colors(colors, scss_path)
    syntax_rules = generate_vscode_syntax(colors, term_colors)
    semantic_rules = generate_vscode_semantic_tokens(colors, term_colors)

    _clean_stale_extensions(fork_key)
    ...
    _write_theme_file(theme_path, workbench_colors, syntax_rules, semantic_rules, colors)
    ...
    if current_theme and current_theme != THEME_NAME:
        settings[PREV_THEME_KEY] = current_theme
        changed = True
...
def strip_vscode_theme(settings_path: str, fork_key: str = "") -> bool:
    """Remove the Ryoku extension and clean generated settings."""
    import shutil
    ...
    changed = False
    current_theme = settings.get("workbench.colorTheme", "")
    previous_theme = settings.get(PREV_THEME_KEY, "")
    if current_theme == THEME_NAME:
        if previous_theme:
            settings["workbench.colorTheme"] = previous_theme
        else:
            settings.pop("workbench.colorTheme", None)
        changed = True
    if PREV_THEME_KEY in settings:
        del settings[PREV_THEME_KEY]
        changed = True
...
def strip_all_vscode_themes(forks: list = None):
    ...
    if forks is None:
        ...
        for key, name in VSCODE_FORKS.items():
            ext_dir = _get_ext_dir(key) / THEME_EXTENSION_ID
            if get_settings_path(name).exists() or ext_dir.exists():
                forks.append(key)
...
def generate_all_vscode_themes(
    ...
    for fork_key in forks:
        ...
        success = generate_vscode_theme(
            colors_json_path, scss_path, str(settings_path), fork_key
        )
        results[fork_key] = success
        if success:
            print(f"  ✓ {fork_name}")
...
if __name__ == "__main__":
    ...
    if args.list_forks:
        ...
        for key, name in VSCODE_FORKS.items():
            ...
            ext_installed = "✓" if (_get_ext_dir(key) / THEME_EXTENSION_ID).exists() else "✗"
            print(f"  [{installed}] {key}: {name} (ext: {ext_installed})")
        sys.exit(0)
