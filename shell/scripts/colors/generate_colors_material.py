#!/usr/bin/env -S\_/bin/sh\_-c\_"source\_\$(eval\_echo\_\${RYOKU_SHELL_VENV:-\$HOME/.local/state/quickshell/.venv})/bin/activate&&exec\_python\_-E\_"\$0"\_"\$@""
import argparse
import math
import json
import os
...
def _auto_detect_scheme(pil_image):
    ...
    rg = np.absolute(r_chan - g_chan)
    yb = np.absolute(0.5 * (r_chan + g_chan) - b_chan)
    colorfulness = float(
        np.sqrt(np.std(rg) ** 2 + np.std(yb) ** 2)
        + 0.3 * np.sqrt(np.mean(rg) ** 2 + np.mean(yb) ** 2)
    )

    r_val, g_val, b_val = r_chan / 255.0, g_chan / 255.0, b_chan / 255.0
    maxc = np.maximum(np.maximum(r_val, g_val), b_val)
    minc = np.minimum(np.minimum(r_val, g_val), b_val)
    saturation_channel = np.where(maxc > 0, (maxc - minc) / maxc, 0.0) * 255
    delta = maxc - minc
    hue_channel = np.zeros_like(r_val)
    with np.errstate(invalid="ignore", divide="ignore"):
        mask = delta > 0
        idx = (maxc == r_val) & mask
        hue_channel[idx] = 30.0 * (((g_val[idx] - b_val[idx]) / delta[idx]) % 6)
        idx = (maxc == g_val) & mask
        hue_channel[idx] = 30.0 * ((b_val[idx] - r_val[idx]) / delta[idx] + 2)
        idx = (maxc == b_val) & mask
        hue_channel[idx] = 30.0 * ((r_val[idx] - g_val[idx]) / delta[idx] + 4)

    saturation = float(np.mean(saturation_channel))
    hue_spread = float(np.std(hue_channel))
...
def readable_hex(fg_hex: str, bg_hex: str, min_ratio: float = 4.5) -> str:
    ...
    return argb_to_hex(
        ensure_contrast(hex_to_argb(fg_hex), bg_argb, min_ratio, is_bg_dark)
    )
