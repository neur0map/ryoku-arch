...
VSCODE_THEMEGEN_BIN="$STATE_DIR/user/generated/bin/ryoku-vscode-themegen"
OPENCODE_THEMEGEN_BIN="$STATE_DIR/user/generated/bin/ryoku-opencode-themegen"
...
NEOVIM_COLORS_FILE="$NEOVIM_CONFIG_DIR/colors/ryoku-shell.lua"
...
# Only deletes if the file was generated by Ryoku (checks for known markers).
strip_neovim_spec() {
  if [[ -f $NEOVIM_THEME_FILE ]] && { grep -Fq 'auto-generated by Ryoku' "$NEOVIM_THEME_FILE" || grep -Fq '"yukazakiri/ryoku.nvim"' "$NEOVIM_THEME_FILE" || grep -Fq '"bjarneo/aether.nvim"' "$NEOVIM_THEME_FILE" || grep -Fq 'name = "ryoku-neovim"' "$NEOVIM_THEME_FILE"; }; then
    rm -f "$NEOVIM_THEME_FILE"
  fi

  if [[ -f $NEOVIM_COLORS_FILE ]] && grep -Fq 'auto-generated by Ryoku' "$NEOVIM_COLORS_FILE"; then
    rm -f "$NEOVIM_COLORS_FILE"
  fi
}
