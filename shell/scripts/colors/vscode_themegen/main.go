...
import (
	...
	common "ryoku/scripts/colors/themegencommon"
)
...
const (
	themeName        = "Ryoku Material"
	themeExtensionID = "ryoku-material-theme"
	themeFileName    = "ryoku-material-color-theme.json"
	prevThemeKey     = "ryoku.previousColorTheme"
)
...
var staleExtensionDirs = []string{"i" + "nir.i" + "nir-theme-1.0.0"}
...
func isDarkTheme(colors map[string]string) bool {
	...
	return hexToHSL(bg).l < 0.5
}

func cleanStaleExtensions(forkKey string) {
	extBase := getExtensionsDir(forkKey)
	for _, stale := range staleExtensionDirs {
		staleDir := filepath.Join(extBase, stale)
...
func ensureExtensionRegistered(forkKey, extDir string) error {
	...
	extID := "ryoku." + themeExtensionID
	for _, entry := range entries {
		identifier, _ := entry["identifier"].(map[string]any)
		if id, _ := identifier["id"].(string); id == extID {
			return nil
		}
	}
...
func main() {
	...
	stripMode := flag.Bool("strip", false, "Remove Ryoku theme extension and clean settings.json")
	var forks stringSlice
	flag.Var(&forks, "forks", "Fork key to target")
	flag.Parse()
	if *stripMode {
	...
	if *listForks {
		...
		for key, name := range vscodeForks {
			...
			extInstalled := "✗"
			if dirExists(filepath.Join(getExtensionsDir(key), themeExtensionID)) {
				extInstalled = "✓"
			}
...
func generateThemeForFork(colorsPath, terminalJSONPath, scssPath, settingsPath, forkKey string) (bool, error) {
	...
	if err := writeThemeFile(filepath.Join(extDir, "themes", themeFileName), colors, termColors); err != nil {
...
func stripThemeForFork(settingsPath, forkKey string) bool {
	...
	changed := false
	currentTheme, _ := settings["workbench.colorTheme"].(string)
	previousTheme, _ := settings[prevThemeKey].(string)
	if currentTheme == themeName {
		if previousTheme != "" {
			settings["workbench.colorTheme"] = previousTheme
		} else {
