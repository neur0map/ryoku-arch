...
post_process() {
    # Best-effort live refresh is handled by applycolor.sh modules.
    return 0
}
...
switch() {
    ...
    if [[ "$color_flag" != "1" ]]; then
        if [[ "$cfg_use_backdrop" == "true" ]]; then
            ...
            if [[ "$cfg_panel_family" == "waffle" ]]; then
                ...
                backdrop_use_main="$cfg_main_backdrop_use_main"
                backdrop_path="$cfg_main_backdrop_path"
            fi
    ...
    # Set terminal generation properties from batched config reads.
    ...
    _ryoku_python="$_ryoku_venv/bin/python3"
    [[ ! -x "$_ryoku_python" ]] && _ryoku_python="python3"
    ...
    if "$_ryoku_python" "$SCRIPT_DIR/generate_colors_material.py" "${generate_colors_material_args[@]}" \
        ...
        if [[ -s "$_palette_tmp" ]]; then
            mv "$_palette_tmp" "$_palette_out"
        else
            rm -f "$_palette_tmp"
        fi
        if [[ -s "$_app_palette_tmp" ]]; then
            mv "$_app_palette_tmp" "$_app_palette_out"
        else
        ...
        if [[ "$force_dark_terminal" == "true" ]]; then
            ...
            if [[ -s "$_terminal_tmp" ]]; then
                mv "$_terminal_tmp" "$_terminal_out"
            else
                rm -f "$_terminal_tmp"
            fi
        fi
        if [[ -s "$_meta_tmp" ]]; then
            mv "$_meta_tmp" "$_meta_out"
        else
            rm -f "$_meta_tmp"
        fi
        if [[ "$force_dark_terminal" != "true" ]]; then
    ...
    if [[ "$force_dark_terminal" == "true" ]]; then
        # Generate material_colors.scss with forced dark mode without darkening the shell/UI palette.
        ...
        scss_cmd=("$_ryoku_python" "$SCRIPT_DIR/generate_colors_material.py" "${scss_generate_args[@]}")
...
main() {
    ...
    # Batch-read all config values in one jq call instead of spawning jq for
    # each option during wallpaper changes.
    if [[ -f "$SHELL_CONFIG_FILE" ]]; then
        mapfile -t _cfg < <(jq -r '[
    ...
    cfg_main_backdrop_use_main="${_cfg[9]:-true}"
    cfg_main_backdrop_path="${_cfg[10]:-}"
    ...
    # auto-detects the scheme from the already-loaded resized image.
