#!/usr/bin/env python3
"""
niri-config.py — Niri compositor configuration helper for Ryoku settings UI.
...
Commands:
  ...
  detect-customizations Report Niri files that differ from shipped Ryoku defaults
        ...
        add_file(
            {
                ...
                "reason": "This managed Niri file differs from the shipped Ryoku default.",
            ...
            add_file(
                {
                    ...
                    if relative_path == "config.d/15-outputs.kdl"
                    else "Additional user config file not shipped by Ryoku defaults.",
                    "preview": preview,
...
def _sync_cursor_env(theme=None, size=None):
    ...
    The install may place XCURSOR_THEME in ryoku-shell.conf or the user might have
    ...
    target_file = None
    for candidate in ["ryoku-shell.conf", "cursor.conf"]:
...
# niri-config.py is called with a full path from the ryoku-shell launcher.
...
def _kb_generate_comment(action: str) -> str:
    ...
    if action.startswith("spawn"):
        ryoku = _kb_parse_ryoku_action(action)
        if ryoku:
            return _KB_IPC_MAP.get(ryoku, f"{ryoku[0]} {ryoku[1]}")
