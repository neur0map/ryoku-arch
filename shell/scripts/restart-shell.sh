...
#
# Delegates to `ryoku-shell restart` which correctly kills the wrapper process
...
launcher_script="$config_dir/scripts/ryoku-shell"

if [[ ! -f "$launcher_script" ]]; then
    launcher_script="$script_dir/ryoku-shell"
fi

if [[ ! -f "$launcher_script" ]]; then
    echo "restart-shell: cannot find ryoku-shell launcher" >&2
    exit 1
fi
