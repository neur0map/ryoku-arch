...
Singleton {
  ...
  readonly property string templateProcessorScript: Quickshell.shellDir + "/settingsgui" + "/Scripts/python/src/theming/template-processor.py"
  ...
  */
  function buildPredefinedTemplateConfig(mode) {
    ...
    TemplateRegistry.terminals.forEach(terminal => {
                                         if (isTemplateEnabled(terminal.id)) {
                                           ...
                                           lines.push(`input_path = "${Quickshell.shellDir + "/settingsgui"}/Assets/Templates/${terminal.predefinedTemplatePath}"`);
  ...
  function addWallpaperTheming(lines, mode) {
    ...
    TemplateRegistry.terminals.forEach(terminal => {
                                         if (isTemplateEnabled(terminal.id)) {
                                           lines.push(`\n[templates.${terminal.id}]`);
                                           lines.push(`input_path = "${Quickshell.shellDir + "/settingsgui"}/Assets/Templates/${terminal.templatePath}"`);
                                           const outputPath = terminal.outputPath.replace("~", homeDir);
                                           lines.push(`output_path = "${outputPath}"`);
                                           const postHookEsc = escapeTomlString(terminal.postHook);
                                           lines.push(`post_hook = "${postHookEsc}"`);
                                            ...
                                            if (app.id === "discord") {
                                                ...
                                                inputs.forEach((inputFile, idx) => {
                                                                 ...
                                                                 app.clients.forEach(client => {
                                                                                       if (isDiscordClientEnabled(client.name)) {
                                                                                         ...
                                                                                         lines.push(`input_path = "${Quickshell.shellDir + "/settingsgui"}/Assets/Templates/${inputFile}"`);
                                                                                         // First input uses legacy name for backward compatibility
                                                                                         const outputFile = idx === 0 ? "ryoku.theme.css" : `ryoku-${themeSuffix}.theme.css`;
                                                ...
                                                app.clients.forEach(client => {
                                                                      ...
                                                                      if (isCodeClientEnabled(client.name) && resolvedPaths.length > 0) {
                                                                        resolvedPaths.forEach((resolvedPath, pathIndex) => {
                                                                                                ...
                                                                                                lines.push(`input_path = "${Quickshell.shellDir + "/settingsgui"}/Assets/Templates/${app.input}"`);
                                              ...
                                              if (isTemplateEnabled("emacs")) {
                                                ProgramCheckerService.availableEmacsClients.forEach(client => {
                                                                                                      ...
                                                                                                      lines.push(`input_path = "${Quickshell.shellDir + "/settingsgui"}/Assets/Templates/${app.input}"`);
                                                                                                      const expandedPath = client.path.replace("~", homeDir) + "/themes/ryoku-theme.el";
  ...
  function buildGenerationScript(content, wallpaper, mode) {
    ...
    let script = `RYOKU_WP_PATH=$(cat << '${wpDelimiter}'\n${wallpaper}\n${wpDelimiter}\n)\n`;
    ...
    if (content) {
      ...
      script += `python3 "${templateProcessorScript}" "$RYOKU_WP_PATH" --scheme-type ${schemeType} --config '${pathEsc}' --default-mode ${mode}\n`;
    }

    script += buildUserTemplateCommand("$RYOKU_WP_PATH", mode);
  ...
  Process {
    id: generateProcess
    workingDirectory: Quickshell.shellDir + "/settingsgui"
    running: false
