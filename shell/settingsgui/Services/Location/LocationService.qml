...
Singleton {
  ...
  property string locationFile: Quickshell.env("RYOKU_WEATHER_FILE") || (Settings.cacheDir + "location.json")
      ...
      if (xhr.readyState === XMLHttpRequest.DONE) {
        if (xhr.status === 200) {
          try {
            ...
            var result = (geoData.results && geoData.results.length > 0) ? geoData.results[0] : null;
            if (result != null && result.latitude != null) {
              var region = result.country || result.admin1 || "";
              callback(result.latitude, result.longitude, result.name, region);
            } else {
  ...
  function taliaWeatherImageFromCode(code) {
    var isDay = data.weather ? data.weather.current_weather.is_day : true;
    if (code >= 40 && code <= 49)
      return Quickshell.shellDir + "/settingsgui" + "/Assets/Talia/TaliaDazed.png";
    if (code >= 95 && code <= 99)
      return Quickshell.shellDir + "/settingsgui" + "/Assets/Talia/TaliaFear.png";
    var wet = (code >= 51 && code <= 67) || (code >= 80 && code <= 82) || (code >= 71 && code <= 77) || (code >= 85 && code <= 86);
    if (wet)
      return Quickshell.shellDir + "/settingsgui" + "/Assets/Talia/TaliaSob.png";
    if ((code === 0 || code === 1 || code === 2) && isDay === false)
      return Quickshell.shellDir + "/settingsgui" + "/Assets/Talia/TaliaVampire.png";
    if ((code === 0 && isDay === true) || code === 1 || code === 2)
      return Quickshell.shellDir + "/settingsgui" + "/Assets/Talia/TaliaParty.png";
    return Quickshell.shellDir + "/settingsgui" + "/Assets/Talia/TaliaBlank.png";
  }
