...
AbstractBackgroundWidget {
    ...
    function _buildList(): var {
        const now = new Date()
        const local = Events.getUpcomingEvents(30).map(e => Object.assign({}, e, { _source: "local" }))
        ...
        Repeater {
            ...
            delegate: RowLayout {
                ...
                Rectangle {
                    ...
                    radius: 1.5
                    color: parent.modelData?.color || parent.modelData?.sourceColor || ColorUtils.applyAlpha(root.colText, 0.5)
                }
    ...
    function _formatDateTime(event): string {
        ...
        if (root.showDate) {
            if (eventDay.getTime() === today.getTime())
                dateStr = Translation.tr("Today")
            else if (eventDay.getTime() === tomorrow.getTime())
                dateStr = Translation.tr("Tomorrow")
            else
                dateStr = Qt.formatDate(dt, "ddd d MMM")
        }
