    ...
    function snip() {
        ...
        switch (root.action) {
            case RegionSelection.SnipAction.Copy:
                snipProc.command = ["/usr/bin/bash", "-c", `_dir='${screenshotSaveDir}' && mkdir -p "$_dir" && _fmt='${StringUtils.shellSingleQuoteEscape(root.screenshotNameFormat)}' && _name="$(date +"$_fmt" 2>/dev/null | sed 's#[/\\\\]#_#g; s#[[:cntrl:]]#_#g')" && [[ -n "$_name" && "$_name" != "." && "$_name" != ".." ]] || _name="ss-$(date +%Y%m%d-%H%M%S)" && _ss="$_dir/$_name.png" && ${cropToStdout} | tee "$_ss" | /usr/bin/wl-copy && echo -n "$_ss" | /usr/bin/wl-copy --primary && ${cleanup} && /usr/bin/notify-send "Screenshot copied" "${rw}x${rh} saved to $_ss" -a "Screenshot" -i camera-photo -t 3000 && (command -v quickshell >/dev/null && quickshell ipc call screenshotEvents captured "Copied to clipboard" "$_ss" || true)`]
                break;
