...
ContentPage {
    ...
    SettingsCardSection {
        ...
        SettingsGroup {
            ...
            StyledText {
                Layout.fillWidth: true
                text: root.hasCustomConfigDiffs
                    ? Translation.tr("These entries differ from Ryoku-managed defaults and may affect update compatibility.")
    ...
    // ============================
    // MUSIC PLAYER (rmpc) EXTRAS
    // ============================
    // Sibling card of Applications above. The slot picker chooses *which*
    // music command launches; this card holds rmpc-specific extras (the
    // MPD daemon toggle, theme regeneration toggle, library folder picker)
    // that don't fit the generic preset model.
    ...
    property bool _rmpcInstalled: false
    Process {
        id: _rmpcProbe
    ...
    Process {
        ...
        onExited: code => {
            if (code !== 0) console.warn("[NiriConfig] daemon-set", _musicDaemonProc.desiredMode, "exit:", code)
        }
    }
    function _applyMusicDaemon(enable) {
        Config.setNestedValue("apps.musicDaemonEnabled", enable)
        _musicDaemonProc.desiredMode = enable ? "on" : "off"
        _musicDaemonProc.command = [root.ryokuBinPath("ryoku-music-daemon-set"), _musicDaemonProc.desiredMode]
        _musicDaemonProc.running = true
    }

    Process {
        id: _musicDirProc
        onExited: code => {
            if (code !== 0) console.warn("[NiriConfig] set-music-dir exit:", code)
        }
    }
    function _applyMusicDir(dir) {
        if (!dir || dir.length === 0) return
        Config.setNestedValue("apps.musicDir", dir)
        _musicDirProc.command = [root.ryokuBinPath("ryoku-mpd-set-music-dir"), dir]
        _musicDirProc.running = true
    }
    ...
    FolderDialog {
        ...
        onAccepted: {
            ...
            if (local && local.length > 0) root._applyMusicDir(local)
        }
    }
    ...
    SettingsCardSection {
        ...
        // Install hint when rmpc isn't on PATH.
        SettingsGroup {
            visible: !root._rmpcInstalled
            ...
            ColumnLayout {
                ...
                RowLayout {
                    ...
                    RippleButton {
                        implicitHeight: 36
                        implicitWidth: _extrasJumpRow.implicitWidth + 20
                        ...
                        contentItem: RowLayout {
                            ...
                            StyledText {
                                text: Translation.tr("Open Extras to install")
                                font.pixelSize: 13
                                color: Appearance.colors.colOnPrimary
                                Layout.alignment: Qt.AlignVCenter
                            }
                        }
                        downAction: () => { GlobalStates.settingsOverlayRequestedPage = 15 }
                        StyledToolTip { text: Translation.tr("Jump to Settings, Extras to install the Music (rmpc + MPD) profile.") }
                    }

                    RippleButton {
                        implicitHeight: 36
                        implicitWidth: _probeRow.implicitWidth + 20
                        buttonRadius: Appearance.rounding.small
                        ...
                        contentItem: RowLayout {
                            ...
                            anchors.centerIn: parent
                            spacing: 6
                            MaterialSymbol { text: "refresh"; iconSize: 16; color: Appearance.colors.colOnLayer1; Layout.alignment: Qt.AlignVCenter }
                            StyledText { text: Translation.tr("Re-check"); font.pixelSize: 12; Layout.alignment: Qt.AlignVCenter }
                        }
                        downAction: () => root._probeRmpc()
                    }
                }
            }
        }

        // Active controls — visible once rmpc is on PATH.
        SettingsGroup {
            visible: root._rmpcInstalled
            ...
            ColumnLayout {
                ...
                StyledText {
                    Layout.fillWidth: true
                    text: Translation.tr("The in-shell media widget surfaces MPD tracks via MPRIS once the daemon is on.")
                    color: Appearance.colors.colSubtext
                    wrapMode: Text.WordWrap
                    font.pixelSize: Appearance.font.pixelSize.small
                }
                ...
                ConfigSwitch {
                    ...
                    onCheckedChanged: {
                        if (checked !== (Config.options?.apps?.musicDaemonEnabled ?? false)) {
                            root._applyMusicDaemon(checked)
                        }
                    }
                    StyledToolTip {
                        text: Translation.tr("Starts and enables mpd.socket via systemctl --user. Off stops and disables both mpd.socket and mpd.service so nothing runs at login.")
                    }
                }
                ...
                ColumnLayout {
                    ...
                    RowLayout {
                        ...
                        RippleButton {
                            implicitHeight: 32
                            implicitWidth: _browseRow.implicitWidth + 16
                            buttonRadius: Appearance.rounding.small
                            colBackground: Appearance.colors.colLayer1
                            colBackgroundHover: Appearance.colors.colLayer1Hover
                            colRipple: Appearance.colors.colLayer1Active
                            contentItem: RowLayout {
                                ...
                                anchors.centerIn: parent
                                spacing: 6
                                MaterialSymbol { text: "folder_open"; iconSize: 16; color: Appearance.colors.colOnLayer1; Layout.alignment: Qt.AlignVCenter }
                                StyledText { text: Translation.tr("Choose..."); font.pixelSize: 12; Layout.alignment: Qt.AlignVCenter }
                            }
                            downAction: () => _musicDirDialog.open()
                            StyledToolTip { text: Translation.tr("Writes music_directory to ~/.config/mpd/mpd.conf and triggers an mpc rescan.") }
                        }
                    }
                }
            }
