...
ContentPage {
    ...
    function launchInTerminal(binName) {
        const terminal = safeTerminal()
        const path = ryokuBinPath(binName)
        if (terminal === "wezterm") {
            Quickshell.execDetached([terminal, "start", "--always-new-process", "--", path])
        } else {
            Quickshell.execDetached([terminal, "-e", path])
        }
    }
    ...
    Process {
        ...
        onExited: code => {
            if (code !== 0) {
                console.warn("[ServicesConfig] ryoku-mpd-set-music-dir failed:", code)
            }
        }
    }
    ...
    SettingsCardSection {
        ...
        SettingsGroup {
                    ...
                    Rectangle {
                        ...
                        RowLayout {
                            ...
                            RippleButton {
                                ...
                                onClicked: {
                                    const model = providerItem.modelData
                                    ...
                                    providerNameInput.text = model?.name ?? ""
                                    providerEndpointInput.text = model?.endpoint ?? ""
                                    providerForm.selectedFormat = model?.api_format ?? "openai"
                                    providerForm._manualOverride = true
                                    providerModelInput.text = model?.model ?? ""
                                    providerApiKeyInput.text = ""
                                    providerForm.expanded = true
                                }
            ...
            ColumnLayout {
                ...
                StyledText {
                    Layout.alignment: Qt.AlignHCenter
                    text: Translation.tr("Add an AI provider to use the Ryoku chat sidebar. Local models via Ollama and free OpenRouter models are loaded automatically.")
                    font.pixelSize: Appearance.font.pixelSize.smallest
                    color: Appearance.colors.colSubtext
                        ...
                        MaterialTextField {
                            ...
                            onTextChanged: {
                                ...
                                if (url.includes("generativelanguage.googleapis.com")) {
                                    ...
                                    providerForm.selectedFormat = "openai-response"
                                } else if (url.includes("api.mistral.ai")) {
                                    providerForm.selectedFormat = "mistral"
                    ...
                    ContentSubsection {
                        ...
                        ConfigSelectionArray {
                            ...
                            options: [
                                ...
                                { displayName: "Mistral", icon: "smart_toy", value: "mistral" },
                    ...
                    RowLayout {
                        ...
                        RippleButton {
                            ...
                            onClicked: {
                                ...
                                const fallbackKeyId = modelCode.toLowerCase().replace(/[:\/ ]/g, "-")
                                const keyId = apiKey.length > 0 ? fallbackKeyId : (original?.key_id ?? fallbackKeyId)
                                ...
                                const entry = {
                                    ...
                                    requires_key: apiKey.length > 0 ? true : (original?.requires_key ?? false),
                                    key_id: keyId,
                                }

                                if (original) {
                                    for (let key in original) {
                                        if (!(key in entry) && key !== "index") entry[key] = original[key]
                                    }
                                }
    ...
    SettingsCardSection {
        ...
        SettingsGroup {
            ColumnLayout {
                ...
                StyledText {
                    Layout.fillWidth: true
                    text: Translation.tr("rmpc is a TUI client for MPD. Install via Settings → Extras → Music (rmpc + MPD). Once installed the shell media widget picks it up via MPRIS automatically.")
                    color: Appearance.colors.colSubtext
                    wrapMode: Text.WordWrap
                    font.pixelSize: 13
                }
                ...
                ColumnLayout {
                    ...
                    RowLayout {
                        ...
                        RippleButton {
                            ...
                            StyledToolTip { text: Translation.tr("Pick a folder. Writes music_directory to ~/.config/mpd/mpd.conf and triggers an mpd rescan.") }
                        }
                    }

                    StyledText {
                        Layout.fillWidth: true
                        text: Translation.tr("Add or remove tracks in this folder; mpd auto-updates on the next launch.")
                        color: Appearance.colors.colSubtext
                        wrapMode: Text.WordWrap
                        font.pixelSize: 12
                    }
