...
Singleton {
    ...
    Process {
        ...
        onExited: {
            root._log("[WindowPreviewService] Loaded", Object.keys(root.previewCache).length, "cached previews")
            root.cleanupOrphans()
        }
    }
    ...
    function _doCapture(): void {
        ...
        root._log("[WindowPreviewService] Capturing", idsToCapture.length, "windows")
        capturing = true
    ...
    function captureAllWindows(): void {
        ...
        root._log("[WindowPreviewService] Force capturing all", windows.length, "windows")
        capturing = true
        Cliphist.suppressRefresh = true
        root._saveClipboard()
    ...
    Process {
        ...
        stdout: SplitParser {
            onRead: (line) => root._log("[WindowPreviewService:capture]", line)
        }
        stderr: SplitParser {
            onRead: (line) => root._log("[WindowPreviewService:capture][err]", line)
        }
