...
Singleton {
    ...
    Process {
        ...
        stdout: StdioCollector {
            onStreamFinished: {
                try {
                    root._setupTargets = JSON.parse(this.text || "[]")
                } catch (e) {
                    root._setupTargets = []
                }
            }
    ...
    function _safeTerminal(): string {
        const terminal = (Config.options?.apps?.terminal ?? "kitty").trim()
        return /^[A-Za-z0-9._+-]+$/.test(terminal) ? terminal : "kitty"
    }

    function _runSetup(target): void {
        const path = `${Directories.scriptsPath}/setup/${target.slug}.sh`
        const terminal = root._safeTerminal()
        Quickshell.execDetached(terminal === "wezterm"
            ? [terminal, "start", "--always-new-process", "--", "/usr/bin/bash", path]
            : [terminal, "-e", "/usr/bin/bash", path])
