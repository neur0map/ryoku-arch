...
if ! curl -sf "https://raw.githubusercontent.com/snowarch/Ryoku/dev/LICENSE" >/dev/null 2>&1; then
...
cp "${PKG_DIR}/inir-shell-git.install" "${WORK_DIR}/ryoku-shell-git.install"
...
ls -la "${WORK_DIR}"/{PKGBUILD,.SRCINFO,ryoku-shell-git.install} 2>/dev/null || \
ls -la "${WORK_DIR}"/{PKGBUILD,.SRCINFO}

# Step 5: Commit and push
echo "[5/5] Committing and pushing to AUR..."
cd "${WORK_DIR}"
git add PKGBUILD .SRCINFO ryoku-shell-git.install 2>/dev/null || git add PKGBUILD .SRCINFO
git commit -m "Initial upload: ryoku-shell-git $(grep pkgver= PKGBUILD | head -1 | cut -d= -f2)"
