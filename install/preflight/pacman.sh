if [[ -n ${RYOKU_ONLINE_INSTALL:-} ]]; then
  ...
  # Configure pacman from the single Ryoku main channel snapshot.
  sudo cp -f "$RYOKU_PATH/default/pacman/pacman-main.conf" /etc/pacman.conf
  sudo cp -f "$RYOKU_PATH/default/pacman/mirrorlist-main" /etc/pacman.d/mirrorlist
