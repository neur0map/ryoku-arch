...
Singleton {
    ...
    Process {
        ...
        command: ["sh", "-c", "command -v cava >/dev/null 2>&1 || exit 127; mon=$(pactl get-default-sink 2>/dev/null).monitor; cfg=\"${XDG_RUNTIME_DIR:-/tmp}/ryoku-cava-launcher.conf\"; printf '%s\\n' '[general]' 'framerate = 30' 'bars = " + root.bars + "' '' '[input]' 'method = pulse' \"source = $mon\" '' '[output]' 'method = raw' 'raw_target = /dev/stdout' 'data_format = ascii' 'ascii_max_range = 100' 'channels = mono' 'mono_option = average' '' '[smoothing]' 'noise_reduction = 45' > \"$cfg\"; exec cava -p \"$cfg\""]
        running: root.active
        stdout: SplitParser {
            splitMarker: "\n"
