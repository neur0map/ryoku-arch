...
Item {
    ...
    signal chosen(bool closeRequested)

    visible: open || fade.running
    opacity: open ? 1 : 0
    Behavior on opacity { NumberAnimation { id: fade; duration: Motion.fast; easing.type: Easing.OutCubic } }
    ...
    function run() {
        ...
        if (!action || action.enabled === false || !action.execute)
            return;
        action.execute();
        root.chosen(action.closeOnExecute !== false);
    }
        ...
        Column {
            ...
            Repeater {
                ...
                delegate: Rectangle {
                    ...
                    width: parent.width
                    height: 30 * root.s
                    radius: MainMetrics.radiusTag * root.s
                    readonly property bool sel: index === root.selectedIndex
                    color: sel ? Theme.threadBg : "transparent"
