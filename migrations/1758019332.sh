...
ryoku-refresh-config uwsm/default
ryoku-refresh-config uwsm/env
ryoku-state set reboot-required
...
if grep -q "bindd = SUPER, N, Neovim" ~/.config/hypr/bindings.conf; then
  sed -i '/SUPER, N, Neovim, exec/ c\bindd = SUPER, N, Editor, exec, ryoku-launch-editor' ~/.config/hypr/bindings.conf
fi
