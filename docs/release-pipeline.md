...
| `R2_BUCKET` | optional | Bucket + prefix to upload into. Defaults to `ryoku/stable`, which is served publicly through `https://iso.ryoku.dev/stable/`. Set to e.g. `ryoku-iso/stable` if you use a different bucket name. |
...
Public artifact URLs (served through Cloudflare CDN via the `iso.ryoku.dev`
custom domain):
