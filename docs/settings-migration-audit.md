...
| `CustomThemeEditor.qml` | 2183 | ~50 color pickers + flows | 🟡 Partial |
...
| Custom Theme Editor (loader) | Appearance > Style: 7 cards | 🟡 Partial | sub-spec #02c MVP: 7 native cards (Custom theme mode+preset, Accent, Secondary, Tertiary, Backgrounds, Borders and shadows, Status), gated on `appearance.theme === "custom"`. Dark/Light segment + 13-preset combo + 24 m3* color pickers wired through `setCustomThemeColor` / `setCustomThemeDarkMode` / `applyCustomThemePreset` helpers. A 350ms debounce timer fires `ThemeService.applyCurrentTheme(false)` locally and IPCs `settings.applyTheme` to the main shell so its `m3colors` re-applies AND `applycolor.sh` regenerates GTK/terminal/Neovim/Chrome/Spicetify/Vesktop/Steam config files in one pass. Deferred to a future sub-spec: color harmony schemes, quick-adjust globals, export/import JSON dialogs, save/load to ~/.config/illogical-impulse/themes/, surface containers (8 extra colors). Plus during this sub-spec: added a "Theme source" segment (Wallpaper / Preset / Custom) inside the Shell style card so the custom-theme entry point is reachable from Style sub-tab, not only from Colors. Also added a fullscreen MouseArea catcher behind every `SettingsCombo` dropdown so clicks outside close the dropdown (single-press passes through to whatever was clicked). Plus the cross-process Config.revision bump in fileReloadTimer (Config.qml) and the `settings.applyTheme` IpcHandler (shell.qml). |
