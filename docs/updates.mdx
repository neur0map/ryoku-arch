...
```bash
curl -fsSL https://raw.githubusercontent.com/neur0map/ryoku-arch/unstable-dev/bin/ryoku-doctor | bash -s -- update
ryoku-update -y
```

This does not require a GitHub account. The downloaded doctor reads the installed Ryoku channel, repairs only old official Ryoku branch history, preserves dirty checkout files in a git stash, and leaves arbitrary local commits untouched.
