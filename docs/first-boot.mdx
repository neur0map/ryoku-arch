...
- **Wire up your VPN.** Settings → Security → OpenVPN profiles. See [security tools](security-tools).
...
Press `Mod+Slash`; if the cheatsheet doesn't open either, the shell isn't running. Drop to a TTY (`Ctrl+Alt+F2`), log in, and run `journalctl --user -u quickshell -n 50` to see why. The [troubleshooting guide](troubleshoot) covers common boot/login issues.

## Where to go next

- [Desktop tour](tour) — Niri's scrollable workspace model and the bar / sidebar surfaces in depth.
- [Keybindings](keybindings) — every shipped binding.
- [Security tools](security-tools) — OpenVPN sidebar, SecPulse, the in-shell polkit agent.
- [Customize](customize) — themes, wallpapers, bar items, keybinds.
