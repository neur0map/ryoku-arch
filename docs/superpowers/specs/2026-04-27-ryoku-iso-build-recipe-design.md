...
- **Cursor renders as the X11 fallback "X" cursor** on first login. Hyprland's `default/hypr/envs.conf` sets `XCURSOR_SIZE` and `HYPRCURSOR_SIZE` but no `XCURSOR_THEME` / `HYPRCURSOR_THEME`. omarchy works around this by calling `omarchy-theme-set "Tokyo Night"` in `install/config/theme.sh`, which side-effects the GSettings cursor theme. Ryoku needs an equivalent step or an explicit theme env so the chosen cursor theme actually loads at session start.
